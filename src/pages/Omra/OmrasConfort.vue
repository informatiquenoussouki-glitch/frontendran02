...
const comfortOmras = computed(() =>
  (omras.value || [])
    .filter(o => unifyType(o) === 'confort' && isPublished(o.status))
...
const monthOptions = computed(() => {
  const set = new Set()
  for (const o of comfortOmras.value) {
...
const filteredOmras = computed(() => {
  if (!selectedMonth.value) return comfortOmras.value
  const target = Number(selectedMonth.value)
  return comfortOmras.value.filter(o => extractMonth(o.date_start) === target)
})

onMounted(async () => {
  try{
    loading.value = true
    // on demande au backend uniquement les “Publier”
...
<template>
  <TopBar :active="3" /> <!-- optionnel: onglet "Omra confort" -->

  <main class="wrap">
    <h1 class="page-title">Omra confort</h1>
    ...
    <p class="muted">Toutes nos formules <strong>publiées</strong> de type <strong>Confort</strong>.</p>

    <p v-if="loading" class="muted">Chargement…</p>
    <p v-else-if="!comfortOmras.length" class="muted">Aucune formule disponible.</p>
