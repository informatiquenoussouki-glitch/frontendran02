      ...
      <header class="hero hero--two" v-if="omra">
        ...
        <div class="hero-left">
          <h1 class="title">{{ omra.title }}
            <span class="badge" :class="`badge--${omra.type}`">
             {{ omra.type === 'economique' ? 'Économique' 
             : omra.type === 'ramadan' ? 'Ramadan' : 'Confort' }}
            </span>
          </h1>
...
<router-link :to="`/reservations/${omra.id}`" class="btn-cta">
      ...
      <section class="program" v-if="programDays.length">
        <h3 class="section-title program-title">
