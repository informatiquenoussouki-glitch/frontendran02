...
// rebuild image URLs coming from storage paths
...
const ecoOmras = computed(() =>
  (omras.value || [])
    .filter(o => unifyType(o) === 'economique' && isPublished(o.status))
...
// ➕ liste des mois disponibles dans les Omra confort
const monthOptions = computed(() => {
  const set = new Set()
  for (const o of ecoOmras.value) {
...
// ➕ omras filtrées par mois
const filteredOmras = computed(() => {
  if (!selectedMonth.value) return ecoOmras.value
  const target = Number(selectedMonth.value)
  return ecoOmras.value.filter(o => extractMonth(o.date_start) === target)
})
...
onMounted(async () => {
  try{
    loading.value = true
    // ask the API for published only (works with your OmraController@index)
...
<template>
  <TopBar :active="2" /> <!-- (optionnel) surligner l’onglet Omra Économique -->

  <main class="wrap">
    <h1 class="page-title">Omra économique</h1>
    ...
    <p class="muted">Toutes nos formules <strong>publiées</strong> de type <strong>Économique</strong>.</p>

    <p v-if="loading" class="muted">Chargement…</p>
    <p v-else-if="!ecoOmras.length" class="muted">Aucune formule disponible.</p>
    <p v-else-if="!filteredOmras.length" class="muted">Aucune formule pour ce mois.</p>

    <div v-else class="grid">
      <router-link
        v-for="o in ecoOmras" :key="o.id"
        class="card"
