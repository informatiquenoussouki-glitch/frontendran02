...
function reset(){
  ...
  previews.value.forEach(u => URL.revokeObjectURL && URL.revokeObjectURL(u))
  previews.value = []
  newFiles.value = []
  if (fileInputRef.value) fileInputRef.value.value = ''
  googleUrl.value = ''
  googleEmbedUrl.value = ''
}
...
<template>
  <main class="content">
    ...
    <form @submit.prevent="save" class="form">
      ...
      <div class="row-1">
        <div class="field">
          <label for="h-distance">Distance au Haram (m) *</label>
          <input id="h-distance"
      ...
      <div class="row-1 actions">
        ...
        <RouterLink class="btn ghost" style="text-align:center" to="/admin/hotels/list">
          Voir la liste
        </RouterLink>
      </div>
