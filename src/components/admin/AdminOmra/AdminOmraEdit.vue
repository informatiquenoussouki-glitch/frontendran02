...
async function onPickFile(e){
  ...
  try{
    ...
    flashError(err?.response?.data?.message || 'Upload impossible')
  }
}
      ...
      <div class="row-2 row-image">
        ...
        <div class="field">
          ...
          <div class="preview-box" 
               :style="{ backgroundImage: form.image_path ? `url(${form.image_path})` : 'none' }">
          </div>
        </div>
      </div>
      ...
      <div class="row-2 hotel-line">
        ...
        <div class="field">
          ...
          <input id="f-medinaNights" v-model.number="form.medina_nights" type="number" min="1"
                 :disabled="!(Number(form.medina_hotel_id) > 0)" :class="{invalid: errors.medina_nights}" />
